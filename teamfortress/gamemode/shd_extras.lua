...
RegisterAttribute("turn_gey", {
	equip = function(v,weapon,owner)
		weapon.CustomColorOverride = Color(255,30,150,255)
		if SERVER then
			weapon.CustomMaterialOverride = "models/shiny"
		else
			weapon.CustomMaterialOverride = Material("models/shiny")
		end
		...
		if CLIENT then
			...
			weapon.DrawWorldModel = function(self,t)
				...
				if IsValid(self.WModel2) then
					self.WModel2:SetMaterial("models/shiny")
				end
			...
			weapon.ViewModelDrawn = function(self,t)
				...
				if IsValid(self.CModel) then
					self.CModel:SetMaterial("models/shiny")
				end
	...
	projectile_fired = function(v,proj,weapon,owner)
		...
		proj:SetColor(255,30,150,255)
		proj.Initialize0 = proj.Initialize
		proj.Initialize = function(self)
			self:Initialize0()
			for _,v in pairs(ents.FindByClass("prop_dynamic")) do
				if v:GetParent()==self then
					v:SetMaterial("models/shiny")
					v:SetColor(255,30,150,255)
				end
			end
		end
	end,
	...
	on_kill = function(v,ent,inf,att)
		--ent:SetNWBool("Gey", true)
		ent:AddDeathFlag(DF_GEY)
	end,
...
local function StopGay(ent)
	if IsValid(ent) then
		ent:SetNWBool("VeryGay", false)
		ent:SetColor(255,255,255,255)
		ent:SetRenderMode( RENDERMODE_TRANSALPHA )
	end
end

local function StartGay(ent)
	ent:SetNWBool("VeryGay", true)
	ent:SetColor(255,40,150,255)
	ent:SetRenderMode( RENDERMODE_TRANSALPHA )
	timer.Simple(8, function() StopGay(ent) end)
end

RegisterAttribute("turn_gey_2", {
	post_damage = function(v,ent,hitgroup,dmginfo)
		if ent ~= dmginfo:GetAttacker() then
			StartGay(ent)
		end
	...
	projectile_fired = function(v,proj,weapon,owner)
		proj.DoExplosion0 = proj.DoExplosion
		proj.DoExplosion = function(self, ent)
			...
			if tr.Hit then
				dir = -1 * tr.HitNormal
				pos = tr.HitPos - dir * 10
				for i=1,10 do
					local ang = Angle(math.Rand(-90,90), math.Rand(-180,180), 0)
					local tr = util.TraceLine{
						start = pos,
					...
					if not tr.Hit then
						tr = util.TraceLine{
							start = tr.HitPos,
							endpos = tr.HitPos + ang:Forward() * 100 + dir * 200,
							filter = self
						}
					end
